...
- **Center Page** (`/center`): Consolidated hub for station operations with 6 tabs: Customers | Bookings | Finance | Forecast | Sales | Incidents. Station leads auto-resolve their school from `assignedStationId`. Customers tab shows list with payment status indicators (green/red dots), customer detail includes booking history with payment update, PDF, and email actions. Bookings has sub-tabs: New Booking and Timeline (Gantt view). Finance tab has time-period filters (Today/Week/Month/Custom), revenue KPI cards (Revenue/Cash/Card/Unpaid), Cash Register with opening balance, Expenses tracker with receipt photo upload and category selection, Net Result calculation, and expandable transaction list from bookings. Forecast tab shows 35-day occupancy bar chart with summary cards (guests today, arrivals/departures this week, open payments). Admins see a school selector dropdown. Supports simulation mode via `simStationId`.
