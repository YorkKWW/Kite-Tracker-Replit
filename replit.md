...
- **Center Page** (`/center`): Consolidated hub for station operations with 5 tabs: Dashboard | Bookings | Customers | Sales | Incidents. Station leads auto-resolve their school from `assignedStationId`. Customers tab shows list with payment status indicators (green/red dots), customer detail includes booking history with payment update, PDF, and email actions. Customer detail also includes a Documents section for scanning and uploading signed documents (AGBs, Stundenzettel) via camera — images are converted to high-contrast B&W scan effect client-side before upload. Documents are stored in Object Storage with category badges and can be viewed full-size or deleted. All logged-in staff can manage documents. Bookings has sub-tabs: New Booking and Timeline (Gantt view). Dashboard tab shows 35-day occupancy bar chart with summary cards (guests today, arrivals/departures this week, open payments). Admins see a school selector dropdown. Supports simulation mode via `simStationId`.
- **Finance Page** (`/finance`): Standalone top-level page with school selector. Features time-period filters (Today/Week/Month/Custom), revenue KPI cards (Revenue/Cash/Card/Unpaid), Cash Register with opening balance and expense deductions (Opening + Cash Income − Expenses = Current Cash), Expenses tracker with receipt photo upload and category selection (fuel_gas, food_drinks, material_supplies, transport, maintenance, staff, other), Net Result calculation, expandable transaction list from bookings, and P&L PDF export (weekly/monthly). Nav order: Center, Finance, Equipment, Settings.
