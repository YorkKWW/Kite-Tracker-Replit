...
- **Center Page** (`/center`): Consolidated hub for station operations with 4 tabs (Buchungen, Kunden, Sales, Incidents). Station leads auto-resolve their school from `assignedStationId`. Buchungen has sub-tabs: Neue Buchung (product tile picker, customer autocomplete + inline creation, custom products) and Übersicht (list/filter/detail/PDF/email). Admins see a school selector dropdown. Supports simulation mode via `simStationId`.
